function SIGO = process_sub_data_pre6(expe,subj,randmats,tex_gains)

S = load(strcat('C:\Users\atrox\Desktop\Work\Research\My projects\Rendering of natural texture\data\subject data\',subj,'\',expe,'_x.mat'));
dur = S.dur;
freqs = S.freqs;
freqsi = S.freqsi;
sr = S.sr;

[b1,a1] = butter(1,15*2/sr,'low');
...
for t = 1:7
    ...
    S = load(strcat('C:\Users\atrox\Desktop\Work\Research\My projects\Rendering of natural texture\data\subject data\',subj,'\',expe,'_',tria,'.mat'));
    dataQ = S.dataQ;
...
inds = 1:length(vels);
velm = mean(vels.');
indsvel = inds(velm>(veld-1));
indsvela = indsvel(1:sr);
indsvelb = indsvel(end-sr+1:end);

s = load(strcat('C:\Users\atrox\Desktop\Work\Research\My projects\Rendering of natural texture\data\subject data\',subj,'\','1','_xx.mat'));
lpfreq = s.lpfreq;
...
for i = 1:49
    SIGO{i} = -5*ones(1,sr*dur).';
    if  randmats{3}(i,3)== 0
        tex1 = textures{randmats{3}(i,2)}(:,1).';
        tex2 = textures{randmats{3}(i,1)}(:,1).';
        if randmats{3}(i,2)<8
            tex1 = tex1*(tex_gains{randmats{3}(i,2)}(end));
        end
        if randmats{3}(i,1)<8
            tex2 = tex2*(tex_gains{randmats{3}(i,1)}(end));
        end
    else
        tex2 = textures{randmats{3}(i,2)}(:,1).';
        tex1 = textures{randmats{3}(i,1)}(:,1).';
        
        if randmats{3}(i,1)<8
            tex1 = tex1*(tex_gains{randmats{3}(i,1)}(end));
        end
        if randmats{3}(i,2)<8
            tex2 = tex2*(tex_gains{randmats{3}(i,2)}(end));
        end        
    end        
    
    tex1 = filter(b,a,tex1)/magP(1);
    tex2 = filter(b,a,tex2)/magP(1);
    
    SIGO{i}(indsvela-sr/8) = tex1;
    SIGO{i}(indsvelb+sr/4) = tex2; % SIGO{i}(indsvelb+sr/8) = tex2; is old line that was applied to subjects <=20
end
